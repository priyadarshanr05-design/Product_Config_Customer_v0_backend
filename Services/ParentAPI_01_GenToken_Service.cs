...
namespace Product_Config_Customer_v0.Services
{
    public class ParentAPI_01_GenToken_Service : IParentAPI_01_GenToken_Service
    {
        ...
        private readonly ILogger<ParentAPI_01_GenToken_Service> _logger;

        private string? _cachedToken;
        private DateTime _tokenExpiry;
        private readonly SemaphoreSlim _tokenLock = new(1, 1);

        public ParentAPI_01_GenToken_Service(
            DomainManagementDbContext dbContext,
            IHttpClientFactory httpClientFactory,
            IConfiguration configuration,
            ILogger<ParentAPI_01_GenToken_Service> logger)
        {
        ...
        public async Task<string> GetBearerTokenAsync(string domainName)
        {
            ...
            try
            {
                ...
                var username = Environment.GetEnvironmentVariable($"EXTERNALMODEL_USERNAME_{domainName}") ?? string.Empty;
                var password = Environment.GetEnvironmentVariable($"EXTERNALMODEL_PASSWORD_{domainName}") ?? string.Empty;

                if (string.IsNullOrWhiteSpace(username) || string.IsNullOrWhiteSpace(password))
                {
                    var err = $"Generate token requires username and password for the domain specified: '{domainName}'";
